#!/bin/sh
##
# Script to deploy a Kubernetes project with a StatefulSet running a MongoDB Sharded Cluster.
##
...
echo -n "  "
until kubectl --v=0 exec mongod-shard0-1 -c mongod-shard0-container -- mongo --quiet --eval 'db.getMongo()'; do
    sleep 5
    echo -n "  "
done
echo -n "  "
...
kubectl exec mongod-configdb-0 -c mongod-configdb-container -- mongo --eval 'rs.initiate({_id: "configserver", version: 1, members: [ {_id: 0, host: "mongod-configdb-0.mongodb-configdb-service.default.svc.cluster.local:27017"}, {_id: 1, host: "mongod-configdb-1.mongodb-configdb-service.default.svc.cluster.local:27017"} ]});'
kubectl exec mongod-shard0-0 -c mongod-shard0-container -- mongo --eval 'rs.initiate({_id: "shard0", version: 1, members: [ {_id: 0, host: "mongod-shard0-0.mongodb-shard0-service.default.svc.cluster.local:27017"}, {_id: 1, host: "mongod-shard0-1.mongodb-shard0-service.default.svc.cluster.local:27017"} ]});'
kubectl exec mongod-shard1-0 -c mongod-shard1-container -- mongo --eval 'rs.initiate({_id: "shard1", version: 1, members: [ {_id: 0, host: "mongod-shard1-0.mongodb-shard1-service.default.svc.cluster.local:27017"}, {_id: 1, host: "mongod-shard1-1.mongodb-shard1-service.default.svc.cluster.local:27017"} ]});'
echo
...
kubectl exec mongod-shard0-0 -c mongod-shard0-container -- mongo --quiet --eval 'while (rs.status().hasOwnProperty("myState") && rs.status().myState != 1) { print("."); sleep(1000); };'
...
kubectl exec mongos-router-0 -c mongos-container -- mongo --eval 'sh.addShard("shard0/mongod-shard0-0.mongodb-shard0-service.default.svc.cluster.local:27017");'
kubectl exec mongos-router-0 -c mongos-container -- mongo --eval 'sh.addShard("shard1/mongod-shard1-0.mongodb-shard1-service.default.svc.cluster.local:27017");'
sleep 3
...
kubectl exec mongos-router-0 -c mongos-container -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"admin",pwd:"admin",roles:[{role:"root",db:"admin"}]});'
echo
